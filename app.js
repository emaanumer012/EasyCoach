...
app.get("/admin/requests",async (req,res)=>{
     const [rows1] = await con.execute(`SELECT * from requests where AdminAction="Pending" order by RequestDate desc`)     
...
app.get("/admin/requests/:request_id/:response",async(req,res)=>{
   ...
   await con.execute(`update requests set AdminAction="${response}" where requestID=${request_id}`)
...
app.get("/student/invoice",async(req,res)=>{
  const [rows] = await con.execute(`SELECT * from payment_requests where AdminAction="Pending"`)
...
app.get("/student/invoice/:p_request_id/:response",async(req,res)=>{
  ...
  if(response=="Unpaid"){
    await con.execute(`update payment_requests set AdminAction="${response}" where P_requestID=${p_request_id}`) 
...
app.get("/driver/salary/:id/:response",async(req,res)=>{
  ...
  await con.execute(`update dsalary set PaidStatus="Paid" where DriverID=${id}`)
...
app.get("/vehicle/manage",async (req,res)=>{
  const [rows] = await con.execute(`select * from vehicle where Enddate is Null`);
...
app.post("/user/profile/:id/payment",async(req,res)=>{
  const {id} =req.params;
  const [admin_account] = await con.execute(`select * from login where UserRole="Administrator"`);
  ...
  const [rows1] = await con.execute(`select * from student where Email="${id}"`);
  const [rows2] = await con.execute("select max(P_requestID) max_id from payment_requests");
  let sql_statement1 = `insert into payment_requests values
                      ("${rows2[0].max_id + 1}","${rows1[0].CmsID}",${req.body.challan_num},"${req.body.transaction_id}"
                      ,"${rows1[0].FirstName} ${rows1[0].LastName}",${req.body.payment_amount},CURDATE(),"Pending")`
  await con.execute(sql_statement1)
...
app.post("/user/profile/:id/request",async(req,res)=>{
  ...
  const [account] = await con.execute(`select * from login where Email="${id}"`)
  if(account[0].UserRole=="Student"){
    ...
    const [rows1] = await con.execute(`select * from Employee where Email="${id}"`);
    let sql_statement1 = `insert into requests(UserRole,UserID,UserName,RequestDate,Information,AdminAction) values
    ...
    const [rows1] = await con.execute(`select * from Driver where Email="${id}"`);
    let sql_statement1 = `insert into requests(UserRole,UserID,UserName,RequestDate,Information,AdminAction) values
...
app.get("/user/profile/:id",isLoggedIn, catchAsync(async (req, res) => {
    ...
    if((account1[0] && account1[0].UserRole=="Student") || (account2[0] && account2[0].UserRole=="Student")){
       ...
       const [rows5] = await con.execute(`select * from requests where UserID="${rows1[0][0].CmsID}" and UserRole="Student" order by RequestDate`)
       const [rows6] = await con.execute(`select * from payment where CmsID="${rows1[0][0].CmsID}"`);
       return res.render("Users/student_profile.ejs", { id ,info:rows1[0][0] ,Driver:rows2[0], 
                                    Route:rows3[0],Destination:rows4[0],request_info:rows5,Payments:rows6}); }
      ...
      const [rows4] = await con.execute(`select * from requests where UserID="${rows1[0][0].DriverID}" and UserRole="Driver" order by RequestDate`)
      return  res.render("Users/driver_profile.ejs",{Driver:rows1[0][0],Route:rows2[0],Dr_details:rows3,request_info:rows4})
    }
      ...
      const [rows3] = await con.execute(`select * from requests where UserID="${rows1[0][0].EmpID}" and UserRole="Employee" order by RequestDate`)
      return res.render("Users/employee_profile.ejs",{Employee:rows1[0][0],Emp_details:rows2,request_info:rows3})
    }else {
