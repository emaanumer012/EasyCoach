...
app.get("/student/invoice", catchAsync(async (req, res) => {
  const [rows] = await con.execute(`SELECT p.*,s.FirstName,s.LastName FROM payment p JOIN student s USING (CmsID);`)
...
app.get(
  ...
  catchAsync(async (req, res) => {
    const [rows] = await con.execute(
      `SELECT d.*,dr.FirstName,dr.LastName FROM dsalary d JOIN driver dr USING (DriverID);`
    );
...
app.get("/employee/salary", catchAsync(async (req, res) => {
    const [rows] = await con.execute(
      `SELECT e.*,em.FirstName,em.LastName FROM esalary e JOIN employee em USING (EmpID);`
    )
...
app.get(
  ...
  catchAsync(async (req, res) => {
    ...
    if (
      ...
      res.render("Users/driver_profile.ejs", {
        Driver: rows1[0][0],
        Route: rows2[0],
        Dr_details: rows3[0],
      });
      ...
      res.render("Users/employee_profile.ejs", {
        Employee: rows1[0][0],
        Emp_details: rows2[0],
      });
    } else {
